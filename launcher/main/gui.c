...
{
    char name[64];
    const char *fallback_subtype = NULL;
    ...
    // Then try exact built-in images
    ...
    if (strcmp(type, "background") == 0 && subtype)
    {
        if (strcmp(subtype, "ws") == 0)
            fallback_subtype = "gb";
        else if (strcmp(subtype, "wsc") == 0)
            fallback_subtype = "gbc";
        else if (strcmp(subtype, "ngp") == 0 || strcmp(subtype, "ngpc") == 0)
            fallback_subtype = "gg";

        if (fallback_subtype)
            snprintf(name, sizeof(name), "%s_%s.png", type, fallback_subtype);
    }
    else if ((strcmp(type, "banner") == 0 || strcmp(type, "logo") == 0) && subtype)
    {
        if (strcmp(subtype, "ngpc") == 0)
            snprintf(name, sizeof(name), "%s_ngp.png", type);
    }

    // Finally use compatible built-in fallbacks
    for (const binfile_t **img = builtin_images; *img; img++)
    {
        if (strcmp((*img)->name, name) == 0)
            return rg_surface_load_image((*img)->data, (*img)->size, 0);
