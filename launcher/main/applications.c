...
static void application_init(retro_app_t *app)
{
    ...
    rg_storage_scandir(app->paths.roms, scan_folder_cb, app,
        strcmp(app->short_name, "snes") == 0 ? RG_SCANDIR_FILES : RG_SCANDIR_RECURSIVE);
    if (strcmp(app->short_name, "snes") != 0)
...
static void application_start(retro_file_t *file, int load_state)
{
    ...
    RG_LOGI("Starting '%s' with app='%s', partition='%s', path='%s'", file->name, name, part, path);
...
static void event_handler(gui_event_t event, tab_t *tab)
{
    ...
    {
        if (file && !tab->preview && gui.idle_counter == 1 && strcmp(app->short_name, "snes") != 0)
...
void applications_init(void)
{
    ...
    application("Sony PlayStation", "psx", "chd cue pbp img iso bin m3u", "psx", 0);
    application("Nintendo Gameboy", "gb", "gb gbc zip", "retro-core", 0);
    application("Nintendo Gameboy Color", "gbc", "gbc gb zip", "retro-core", 0);
    application("Nintendo Gameboy Advance", "gba", "gba", "gbsp", 0);
    application("Nintendo Game & Watch", "gw", "gw", "retro-core", 0);
    application("Bandai WonderSwan", "ws", "ws zip", "oswan", 0);
    application("Bandai WonderSwan Color", "wsc", "wsc zip", "oswan", 0);
    ...
    application("Neo Geo Pocket Color", "ngp", "ngp ngc zip", "ngp", 0);
