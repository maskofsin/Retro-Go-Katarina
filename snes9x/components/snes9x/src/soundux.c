...
#define ECHO_BUFFER_SAMPLES 16000
static struct {
...
static int32_t *Echo; // (512 * 0xF * so.playback_rate) / 32040 * 2
...
void S9xSetEchoEnable(uint8_t byte)
{
   ...
   {
      memset(Echo, 0, ECHO_BUFFER_SAMPLES * sizeof(*Echo));
...
void S9xSetEchoDelay(int32_t delay)
{
   ...
   if (SoundData.echo_buffer_size > ECHO_BUFFER_SAMPLES)
      SoundData.echo_buffer_size = ECHO_BUFFER_SAMPLES;
...
bool S9xInitSound(int32_t buffer_ms, int32_t lag_ms)
{
   ...
   Echo = rg_alloc(ECHO_BUFFER_SAMPLES * sizeof(*Echo), MEM_SLOW);
   if (!LocalState || !Echo)
      return false;
