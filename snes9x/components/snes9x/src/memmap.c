...
char S9xLoadError[160];

static void SetLoadError(const char *fmt, ...)
{
   va_list args;
   va_start(args, fmt);
   vsnprintf(S9xLoadError, sizeof(S9xLoadError), fmt, args);
   va_end(args);
   RG_LOGE("%s", S9xLoadError);
}
...
{
   Memory.RAM   = (uint8_t*)rg_alloc(RAM_SIZE, MEM_FAST);
   Memory.SRAM  = (uint8_t*)rg_alloc(SRAM_SIZE, MEM_SLOW);
   Memory.VRAM  = (uint8_t*)rg_alloc(VRAM_SIZE, MEM_FAST);
   Memory.FillRAM = (uint8_t*)rg_alloc(0x8000, MEM_FAST);

   Memory.Map = (uint8_t**)rg_alloc(MEMMAP_NUM_BLOCKS * sizeof(uint8_t*), MEM_FAST);
   Memory.MapInfo = (SMapInfo*)rg_alloc(MEMMAP_NUM_BLOCKS * sizeof(SMapInfo), MEM_FAST);

   IPPU.ScreenColors = (uint16_t *)rg_alloc(256 * 9 * sizeof(uint16_t), MEM_FAST);
   IPPU.DirectColors = IPPU.ScreenColors + 256;
   IPPU.TileCache = (uint8_t*) rg_alloc(MAX_2BIT_TILES * 128, MEM_FAST);
   IPPU.TileCached = (uint8_t*) rg_alloc(MAX_2BIT_TILES, MEM_FAST);

   bytes0x2000 = (uint8_t *)rg_alloc(0x2000, MEM_FAST);
...
bool LoadROM(const char* filename)
{
   ...
   if (filename == NULL)
   {
      ...
      RG_LOGI("ROM load source=memory size=%u buffer=%u", (unsigned)TotalFileSize, (unsigned)Memory.ROM_AllocSize);
   }
   else if ((fp = fopen(filename, "rb")))
   {
      ...
      fseek(fp, 0, SEEK_SET);
      RG_LOGI("ROM load source=file size=%u buffer=%u", (unsigned)TotalFileSize, (unsigned)Memory.ROM_AllocSize);
      fread(Memory.ROM, 1, TotalFileSize > Memory.ROM_AllocSize ? Memory.ROM_AllocSize : TotalFileSize, fp);
      fclose(fp);
   }
   else
   {
      SetLoadError("ROM open failed: %s", filename);
      return false;
   }

   if (TotalFileSize > Memory.ROM_AllocSize)
   {
      SetLoadError("ROM too big: %u bytes > %u byte buffer", (unsigned)TotalFileSize, (unsigned)Memory.ROM_AllocSize);
      TotalFileSize = Memory.ROM_AllocSize;
      return false; // comment to try to run it anyway
   }
   else if (TotalFileSize < 1024)
   {
      SetLoadError("ROM too small or empty: %u bytes", (unsigned)TotalFileSize);
...
void InitROM(bool Interleaved)
{
   ...
   /* DSP1/2/3/4 */
   if (!S9xStandardRomMode && Memory.ROMType == 0x03)
   {
   ...
   else if (!S9xStandardRomMode && Memory.ROMType == 0x05)
   {
   ...
   if (Memory.HiROM)
   {
      ...
      Settings.SRTC = !S9xStandardRomMode && ((Memory.ROMType & 0xf0) >> 4) == 5;

      if (!S9xStandardRomMode && ((Memory.ROMSpeed & 0x0F) == 0x0A) && ((Memory.ROMType & 0xF0) == 0xF0))
      {
   ...
   else
   {
      ...
      if (!S9xStandardRomMode && Memory.ROMType == 0x25)
         Settings.OBC1 = true;

      if (!S9xStandardRomMode && (Memory.ROMType & 0xf0) == 0x10)
         Settings.SuperFX = !Settings.ForceNoSuperFX;

      /* OBC1 hack ROM */
      if (!S9xStandardRomMode && strncmp(Memory.ROMName, "METAL COMBAT", 12) == 0 && Memory.ROMType == 0x13 && Memory.ROMSpeed == 0x42)
      {
         Settings.OBC1 = true;
         Settings.SuperFX = Settings.ForceSuperFX;
      ...
      if (!S9xStandardRomMode && (Memory.ROMType & 0xf0) == 0x40)
      ...
      if (!S9xStandardRomMode && (Memory.ROMType & 0xf0) == 0xf0 && (strncmp(Memory.ROMName, "MEGAMAN X", 9) == 0 || strncmp(Memory.ROMName, "ROCKMAN X", 9) == 0))
