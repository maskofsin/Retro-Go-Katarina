...
bool S9xFastTransparency = true;
bool S9xFastFrameSkip = true;
bool S9xFastEffects = true;
bool S9xFastSprites = true;
static bool S9xHiResCompatEffects = false;
...
((!S9xFastTransparency || S9xHiResCompatEffects) && \
 (GFX.r2130 & 0x30) != 0x30 && \
 (GFX.r2130 & 2) && \
 (GFX.r212d & 0x1f))

#define ADD_OR_SUB_ON_ANYTHING \
((!S9xFastTransparency || S9xHiResCompatEffects) && (GFX.r2131 & 0x3f))
...
static inline bool AnyClipWindowsEnabled(void)
{
   if (Memory.FillRAM[0x2130] & 0xf0)
      return true;

   for (int32_t w = 0; w < 6; w++)
   {
      if (PPU.ClipWindow1Enable[w] || PPU.ClipWindow2Enable[w])
         return true;
   }
...
static INLINE void SelectHiResTileRenderer(void)
{
...
{
   LocalState = rg_alloc(sizeof(*LocalState), MEM_FAST);
   if (!LocalState)
      return false;
...
void S9xStartScreenRefresh(void)
{
   if (IPPU.RenderThisFrame)
   {
      ...
      bool hires_mode = PPU.BGMode == 5 || PPU.BGMode == 6;

      if (hires_mode)
         IPPU.Interlace = (Memory.FillRAM[0x2133] & 1);
      if (hires_mode || (!S9xFastEffects && IPPU.Interlace))
      {
         IPPU.RenderedScreenWidth = 256;
         IPPU.DoubleWidthPixels = false;
         IPPU.HalfWidthPixels = true;
         SelectHiResTileRenderer();
...
void S9xSetupOBJ(void)
{
   ...
   ObjScreenHeight = S9xFastSprites ? PPU.ScreenHeight : SNES_HEIGHT_EXTENDED;
...
static void DrawOBJS(bool OnMain, uint8_t D)
{
   ...
   if (PPU.BGMode == 5 || PPU.BGMode == 6)
   {
      ...
      if (!IPPU.HalfWidthPixels && IPPU.DoubleHeightPixels)
      {
   ...
   if (!clipcount)
   {
      ...
      {
         int32_t I = 0;
         int32_t tiles = S9xFastSprites ? 0 : GFX.OBJLines[Y].Tiles;
         int32_t S;
         for (S = GFX.OBJLines[Y].OBJ[I].Sprite; S >= 0 && I < 32; S = GFX.OBJLines[Y].OBJ[++I].Sprite)
         {
            ...
            if (!S9xFastTransparency && OnMain && SUB_OR_ADD(4))
            ...
            if (S9xFastSprites)
            {
               for (O = Offset + X * GFX.PixSize; X < 256 && X < PPU.OBJ[S].HPos + GFX.OBJWidths[S]; TileX = (TileX + TileInc) & 0x0f, X += 8, O += 8 * GFX.PixSize)
               {
                  if (X < -7)
                     continue;
                  if (X < 0)
...
static void DrawBackgroundMode5(uint32_t bg, uint8_t Z1, uint8_t Z2)
{
   ...
   bool render_interlace = IPPU.Interlace && !IPPU.HalfWidthPixels;
   ...
   endy = render_interlace ? 1 + (GFX.EndY << 1) : GFX.EndY;

   for (Y = render_interlace ? GFX.StartY << 1 : GFX.StartY; Y <= endy; Y += Lines)
   {
      int32_t ScreenLine;
      int32_t t1;
      int32_t t2;
      ...
      int32_t clipcount;
      int32_t clip;
      int32_t y = render_interlace ? (Y >> 1) : Y;
      uint32_t VOffset = LineData [y].BG[bg].VOffset;
      uint32_t HOffset = LineData [y].BG[bg].HOffset;
...
static void DrawBackground(uint32_t BGMode, uint32_t bg, uint8_t Z1, uint8_t Z2)
{
   ...
   BG.DirectColourMode = (!S9xFastEffects || S9xHiResCompatEffects) && (BGMode == 3 || BGMode == 4) && bg == 0 && (GFX.r2130 & 1);

   if ((!S9xFastEffects || S9xHiResCompatEffects) && PPU.BGMosaic [bg] && PPU.Mosaic > 1)
   {
...
void S9xUpdateScreen(void)
{
   ...
   bool hires_mode = PPU.BGMode == 5 || PPU.BGMode == 6;
   S9xHiResCompatEffects = hires_mode;
   GFX.Pseudo = false;
   ...
   if (PPU.RecomputeClipWindows)
   {
      ...
      {
         memset(IPPU.Clip[0].Count, 0, sizeof(IPPU.Clip[0].Count));
         memset(IPPU.Clip[1].Count, 0, sizeof(IPPU.Clip[1].Count));
      }
   ...
   if (!hires_mode && (IPPU.HalfWidthPixels || IPPU.DoubleWidthPixels))
   {
      IPPU.RenderedScreenWidth = 256;
      IPPU.DoubleWidthPixels = false;
      IPPU.HalfWidthPixels = false;
   ...
   if (hires_mode || (!S9xFastEffects && (IPPU.Interlace || IPPU.DoubleHeightPixels)))
   {
      if (hires_mode || IPPU.Interlace)
      {
         IPPU.RenderedScreenWidth = 256;
         x2 = 1;
      }
      ...
      if (!IPPU.HalfWidthPixels && hires_mode && !IPPU.DoubleWidthPixels)
      {
         IPPU.DoubleWidthPixels = false;
         IPPU.HalfWidthPixels = true;
         SelectHiResTileRenderer();
       ...
       *     too. */
      if (!IPPU.HalfWidthPixels && IPPU.Interlace && !IPPU.DoubleHeightPixels)
      {
         int32_t y;
   ...
   else
   {
      ...
      if (!PPU.ForcedBlanking)
      {
         if (GFX.ZPitch == (uint32_t)IPPU.RenderedScreenWidth)
            memset(GFX.ZBuffer + starty * GFX.ZPitch, 0, (endy - starty + 1) * GFX.ZPitch);
         else
         {
            uint32_t y;
            for (y = starty; y <= endy; y++)
