...
static u32 INT_ATTR fetchICache(psxRegisters *regs, const uintptr_t *memRLUT, u32 pc)
{
	if (pc < 0xa0000000)
	{
		struct cache_entry *entry = &ICache[(pc & 0x5ff0) >> 4];
...
static IRAM_ATTR void intExecute(psxRegisters *regs) {
	u32 (INT_ATTR *fetch)(struct psxRegisters *r, const uintptr_t *luts, u32 pc) =
		regs->ptrs.intFetch;
...
static IRAM_ATTR void intExecuteBp(psxRegisters *regs) {
	u32 (INT_ATTR *fetch)(struct psxRegisters *r, const uintptr_t *luts, u32 pc) =
		regs->ptrs.intFetch;
	const uintptr_t *memRLUT = regs->ptrs.memRLUT;
...
static IRAM_ATTR void intExecuteBlock(psxRegisters *regs, enum blockExecCaller caller) {
	u32 (INT_ATTR *fetch)(struct psxRegisters *r, const uintptr_t *luts, u32 pc) =
		regs->ptrs.intFetch;
	const uintptr_t *memRLUT = regs->ptrs.memRLUT;
...
static IRAM_ATTR void intExecuteBlockBp(psxRegisters *regs, enum blockExecCaller caller) {
	u32 (INT_ATTR *fetch)(struct psxRegisters *r, const uintptr_t *luts, u32 pc) =
		regs->ptrs.intFetch;
	const uintptr_t *memRLUT = regs->ptrs.memRLUT;
