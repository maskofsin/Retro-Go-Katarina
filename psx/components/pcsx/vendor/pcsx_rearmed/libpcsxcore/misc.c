...
#define READTRACK() \
	do { \
		int read_ret = cdra_readTrack(time); \
		if (read_ret) { \
			SysMessage("PSX disc read failed at %02d:%02d:%02d (%d).", time[0], time[1], time[2], read_ret); \
			return -1; \
		} \
		buf = cdra_getBuffer(); \
		if (buf == NULL) { \
			SysMessage("PSX disc read returned no data at %02d:%02d:%02d.", time[0], time[1], time[2]); \
			return -1; \
		} \
...
int LoadCdrom() {
	...
	if (!Config.HLE) {
		...
		if (psxRegs.pc != 0x80030000)
			SysPrintf("ESP32 forcing direct CD boot from BIOS pc=%08x\n", psxRegs.pc);
#endif
	}
...
int CheckCdrom() {
	...
	if (GetCdromFile(mdir, time, "SYSTEM.CNF;1") != -1) {
		...
		if (GetCdromFile(mdir, time, exename) == -1) {
			...
			if (GetCdromFile(mdir, time, exename) == -1) {
				...
				if (ptr != NULL) {
					...
					*ptr = '\0';
					if (GetCdromFile(mdir, time, exename) == -1) {
						SysMessage("PSX boot file not found: %s", exename);
					 	return -1;		// main executable not found
					}
				} else {
					SysMessage("PSX SYSTEM.CNF has no cdrom boot path.");
					return -1;
				}
			}
		}
		...
		SysMessage("PSX disc has no SYSTEM.CNF or PSX.EXE.");
