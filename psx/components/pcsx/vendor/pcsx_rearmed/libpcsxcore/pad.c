...
extern void pcsx_runtime_sync_input(void);
#endif
...
static void initBufForRequest(PadDataS *pad, unsigned char value)
{
	...
	if (0 && (u32)(frame_counter - pad->ds.lastUseFrame) > 2*60u
	...
	if (0 && pad->ds.padMode == 0 && value == CMD_READ_DATA_AND_VIBRATE
		 && pad->ds.configModeUsed
		 && !pad->ds.configMode
		 && !pad->ds.userToggled)
	{
...
static unsigned char PADpoll_(int port, unsigned char value, int pos, int *more_data)
{
	...
	if ((unsigned int)port < 8) {
		pad->buttonStatus = ~in_keystate[port];
		pad->rxData[2] = pad->buttonStatus & 0xff;
		pad->rxData[3] = pad->buttonStatus >> 8;
	}
#endif
