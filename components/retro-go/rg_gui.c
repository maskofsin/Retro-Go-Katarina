...
void rg_gui_draw_icons(void)
{
    ...
    if (network.state > RG_NETWORK_DISCONNECTED)
    {
        ...
        // Show IP address next to WiFi icon when connected
        if (network.state == RG_NETWORK_CONNECTED && network.ip_addr[0]) {
            right += 2;
            int ip_x = -(right);
            rg_gui_draw_text(ip_x, 0, 0, network.ip_addr, C_CYAN, C_TRANSPARENT, 0);
            right += TEXT_RECT(network.ip_addr, 0).width + 2;
        }
    }
...
void rg_gui_debug_menu(void)
{
...
#ifdef RG_ENABLE_NETWORKING
        {0x200, "Show IP",      NULL,         RG_DIALOG_FLAG_NORMAL, NULL},
#endif
    ...
    case 0x200:
    {
        ...
        if (net.state == RG_NETWORK_CONNECTED)
            snprintf(msg, sizeof(msg), "SSID: %s\nIP: %s", net.name, net.ip_addr);
        else
            snprintf(msg, sizeof(msg), "Not connected\nState: %d", net.state);
        rg_gui_alert("Network Info", msg);
        break;
    }
    case 0x100:
