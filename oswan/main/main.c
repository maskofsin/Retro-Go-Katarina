...
void app_main(void) {
  ...
  if (rg_extension_match(app->romPath, "zip")) {
    ...
    if (!fp) RG_PANIC("ROM file not found!");
    fseek(fp, 0, SEEK_END);
    rom_size = ftell(fp);
    fseek(fp, 0, SEEK_SET);
    rom_ptr = malloc(rom_size);
    if (!rom_ptr || !fread(rom_ptr, rom_size, 1, fp)) {
      if (rom_ptr) free(rom_ptr);
      fclose(fp);
  ...
  // Do NOT free rom_ptr - OSwan stores pointers into it for ROM banking
  ...
  for (;;) {
    ...
    // app->quit not available in stock API, event handler calls rg_system_exit
  }
