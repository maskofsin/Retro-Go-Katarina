...
if os.path.exists(f"components/retro-go/targets/{args.target}/env.py"):
    ...
    with open(f"components/retro-go/targets/{args.target}/env.py", "rb") as f:
        ...
        exec(f.read(), env_scope)
        if "IDF_TARGET" in env_scope:
            IDF_TARGET = env_scope["IDF_TARGET"]
        if "DEFAULT_APPS" in env_scope:
            DEFAULT_APPS = env_scope["DEFAULT_APPS"]
