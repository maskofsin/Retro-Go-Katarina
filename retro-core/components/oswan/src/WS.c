...
BYTE *IO;                   // IO
BYTE gDummyPage[0x10000];   // Open-bus dummy page for unmapped banks
BYTE *MemDummy = gDummyPage;
...
void  WriteIO(DWORD A, BYTE V)
{
    ...
    case 0xC0:
        ...
        j = (V << 4) & 0xF0;
        Page[0x4] = WsGetRomBank(0x4 | j, 0x4);
        Page[0x5] = WsGetRomBank(0x5 | j, 0x5);
        Page[0x6] = WsGetRomBank(0x6 | j, 0x6);
        Page[0x7] = WsGetRomBank(0x7 | j, 0x7);
        Page[0x8] = WsGetRomBank(0x8 | j, 0x8);
        Page[0x9] = WsGetRomBank(0x9 | j, 0x9);
        Page[0xA] = WsGetRomBank(0xA | j, 0xA);
        Page[0xB] = WsGetRomBank(0xB | j, 0xB);
        Page[0xC] = WsGetRomBank(0xC | j, 0xC);
        Page[0xD] = WsGetRomBank(0xD | j, 0xD);
        Page[0xE] = WsGetRomBank(0xE | j, 0xE);
        Page[0xF] = WsGetRomBank(0xF | j, 0xF);
        break;
    case 0xC1:
    ...
    break;
    case 0xC2:
        Page[2] = WsGetRomBank(V, 2);
        break;
    case 0xC3:
        Page[3] = WsGetRomBank(V, 3);
        break;
    case 0xC8:
...
void WsReset (void)
{
    ...
    {
        Page[0x1] = WsGetRomBank(0x00, 0x1);
        sCEep.data = NULL;
        sCEep.we = 0;
    }
    Page[0xF] = WsGetRomBank(0xFF, 0xF);
    i = (SPRTAB & 0x1F) << 9;
    i += SPRBGN << 2;
    j = SPRCNT << 2;
