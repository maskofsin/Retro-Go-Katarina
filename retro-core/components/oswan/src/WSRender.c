...
void AllocateBuffers(void) {
    ...
    RG_ASSERT(Palette, "WS Palette allocation failed");

    // SprTMap : 512 bytes
    SprTMap = (BYTE*)malloc(512 * sizeof(BYTE));
    RG_ASSERT(SprTMap, "WS SprTMap allocation failed");
    memset(SprTMap, 0, 512 * sizeof(BYTE));

    // FrameBuffer : LINE_SIZE * LCD_MAIN_H WORDs
    FrameBuffer = (WORD*)rg_alloc(LINE_SIZE * LCD_MAIN_H * sizeof(WORD), MEM_SLOW);
    RG_ASSERT(FrameBuffer, "WS framebuffer allocation failed");
