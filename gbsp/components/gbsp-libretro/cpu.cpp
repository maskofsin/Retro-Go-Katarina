...
#ifndef ENABLE_THUMB_BLOCK_INTERPRETER
#define ENABLE_THUMB_BLOCK_INTERPRETER 1
#endif

#ifndef THUMB_BLOCK_CACHE_SIZE
#define THUMB_BLOCK_CACHE_SIZE 2048
#endif

#ifndef THUMB_BLOCK_MAX_OPS
#define THUMB_BLOCK_MAX_OPS 64
#endif

#ifndef THUMB_BLOCK_CHAIN_LIMIT
#define THUMB_BLOCK_CHAIN_LIMIT 8
#endif

#ifndef THUMB_BLOCK_PROFILE
#define THUMB_BLOCK_PROFILE 0
#endif

#ifdef RETRO_GO
#ifndef THUMB_BLOCK_CACHE_MEM
#define THUMB_BLOCK_CACHE_MEM MEM_ANY
#endif
#endif

#if (THUMB_BLOCK_CACHE_SIZE & (THUMB_BLOCK_CACHE_SIZE - 1)) != 0
#error THUMB_BLOCK_CACHE_SIZE must be a power of two
#endif

#define THUMB_BLOCK_CACHE_MASK (THUMB_BLOCK_CACHE_SIZE - 1)
...
#define check_pc_region()                                                     \
  ...
  if (__builtin_expect(new_pc_region == pc_region, 1)) {}                     \
  else                                                                        \
  {                                                                           \
...
#if !defined(HAVE_DYNAREC) && ENABLE_THUMB_BLOCK_INTERPRETER

enum thumb_block_exit_type
{
  THUMB_BLOCK_EXIT_NONE = 0,
  THUMB_BLOCK_EXIT_RAN = 1,
  THUMB_BLOCK_EXIT_ARM = 2
};

enum thumb_block_handler_type
{
  TBH_LSL_IMM,
  TBH_LSR_IMM,
  TBH_ASR_IMM,
...
struct thumb_block_op
{
  u16 opcode;
  s16 simm;   /* immediate, signed offset, or register list */
  u8 handler;
  u8 rd;
  u8 rs;
  u8 rn;
...
static_assert(sizeof(thumb_block_op) == 8, "thumb_block_op must stay compact");
...
struct thumb_block
{
  ...
  u16 op_count;
  u16 terminal;
  thumb_block_op ops[THUMB_BLOCK_MAX_OPS];
};

static thumb_block *thumb_block_cache = NULL;

#ifndef THUMB_BLOCK_RAM_CODE_EWRAM
#define THUMB_BLOCK_RAM_CODE_EWRAM 0x01
#endif
#ifndef THUMB_BLOCK_RAM_CODE_IWRAM
#define THUMB_BLOCK_RAM_CODE_IWRAM 0x02
#endif
extern "C" { u32 thumb_block_ram_code_region_mask = 0; }

#define THUMB_BLOCK_RAM_PAGE_SHIFT 9
#define THUMB_BLOCK_EWRAM_PAGES (0x40000 >> THUMB_BLOCK_RAM_PAGE_SHIFT)
#define THUMB_BLOCK_IWRAM_PAGES (0x08000 >> THUMB_BLOCK_RAM_PAGE_SHIFT)
#define THUMB_BLOCK_RAM_PAGE_COUNT (THUMB_BLOCK_EWRAM_PAGES + THUMB_BLOCK_IWRAM_PAGES)
#define THUMB_BLOCK_RAM_PAGE_WORDS ((THUMB_BLOCK_RAM_PAGE_COUNT + 31) / 32)

static u32 thumb_block_ram_page_generation[THUMB_BLOCK_RAM_PAGE_COUNT];
static u32 thumb_block_ram_page_has_code[THUMB_BLOCK_RAM_PAGE_WORDS];
static bool thumb_block_cache_ready = false;

#if THUMB_BLOCK_PROFILE
struct thumb_block_stats_type
{
  u32 cache_hits;
  u32 cache_misses;
  u32 blocks_built;
...
static thumb_block_stats_type thumb_block_stats;
#define TB_STAT_INC(field) do { thumb_block_stats.field++; } while (0)
#define TB_STAT_ADD(field, value) do { thumb_block_stats.field += (value); } while (0)
#else
#define TB_STAT_INC(field) do { } while (0)
#define TB_STAT_ADD(field, value) do { } while (0)
#endif

static inline s16 thumb_block_ram_page_index(u32 address)
{
  switch (address >> 24)
  {
    case 0x02:
      return (s16)((address & 0x3FFFF) >> THUMB_BLOCK_RAM_PAGE_SHIFT);
    case 0x03:
      return (s16)(THUMB_BLOCK_EWRAM_PAGES +
                   ((address & 0x07FFF) >> THUMB_BLOCK_RAM_PAGE_SHIFT));
    default:
      return -1;
  }
}

static inline bool thumb_block_ram_page_has_cached_code(s16 page)
{
  return page >= 0 &&
    ((thumb_block_ram_page_has_code[page >> 5] >> (page & 31)) & 1);
}

static inline void thumb_block_mark_ram_page(s16 page)
{
  if (page >= 0)
  {
    thumb_block_ram_page_has_code[page >> 5] |= (1U << (page & 31));
    thumb_block_ram_code_region_mask |=
      (page < THUMB_BLOCK_EWRAM_PAGES) ?
      THUMB_BLOCK_RAM_CODE_EWRAM : THUMB_BLOCK_RAM_CODE_IWRAM;
  }
}

static inline void thumb_block_invalidate_page(s16 page)
{
  if (!thumb_block_ram_page_has_cached_code(page))
    return;

  u32 next = thumb_block_ram_page_generation[page] + 1;
  if (next == 0)
    next = 1;
  thumb_block_ram_page_generation[page] = next;
  TB_STAT_INC(invalidations);
}

static void thumb_block_invalidate_region_pages(u32 region,
                                                u32 start_offset,
                                                u32 end_offset)
{
  u32 page_base;
  ...
  {
    page_base = 0;
    page_count = THUMB_BLOCK_EWRAM_PAGES;
    region_mask = 0x3FFFF;
  }
  else if (region == 0x03)
  {
    page_base = THUMB_BLOCK_EWRAM_PAGES;
    page_count = THUMB_BLOCK_IWRAM_PAGES;
    region_mask = 0x07FFF;
  }
  else
  {
  ...
  const u32 page0 = (start_offset & region_mask) >> THUMB_BLOCK_RAM_PAGE_SHIFT;
  const u32 page1 = (end_offset & region_mask) >> THUMB_BLOCK_RAM_PAGE_SHIFT;

  if (page0 <= page1)
  {
    for (u32 page = page0; page <= page1; page++)
      thumb_block_invalidate_page((s16)(page_base + page));
  }
  else
  {
    /* EWRAM/IWRAM mirror inside their 16 MB address windows. A DMA range can
     * cross the mirror end and wrap back to page zero while the high byte stays
     * the same. Invalidate both sides instead of silently missing the wrap. */
    for (u32 page = page0; page < page_count; page++)
      thumb_block_invalidate_page((s16)(page_base + page));
    for (u32 page = 0; page <= page1; page++)
      thumb_block_invalidate_page((s16)(page_base + page));
  }
}

extern "C" void thumb_block_invalidate_address(u32 address)
{
  thumb_block_invalidate_range(address, 4);
}

extern "C" void thumb_block_invalidate_range(u32 address, u32 bytes)
{
  if (bytes == 0)
    return;

  const u32 end = address + bytes - 1;

  if (end < address)
  {
    /* Extremely defensive: a wrapped range could have touched both executable
     * RAM windows. This path should be rare, so prefer correctness. */
    thumb_block_invalidate_region_pages(0x02, 0, 0x3FFFF);
    thumb_block_invalidate_region_pages(0x03, 0, 0x07FFF);
    return;
  }

  const u32 start_region = address >> 24;
  const u32 end_region = end >> 24;

  if (start_region == end_region)
  {
    thumb_block_invalidate_region_pages(start_region, address, end);
    return;
  }

  for (u32 region = start_region; region <= end_region; region++)
  {
    if (region == 0x02)
    {
      const u32 first = (region == start_region) ? address : 0;
      const u32 last = (region == end_region) ? end : 0x02FFFFFF;
      thumb_block_invalidate_region_pages(0x02, first, last);
    }
    else if (region == 0x03)
    {
      const u32 first = (region == start_region) ? address : 0;
      const u32 last = (region == end_region) ? end : 0x03FFFFFF;
      thumb_block_invalidate_region_pages(0x03, first, last);
    }
  }
}

extern "C" void thumb_block_flush_cache(void)
{
  if (thumb_block_cache)
  {
    memset(thumb_block_cache, 0, sizeof(thumb_block) * THUMB_BLOCK_CACHE_SIZE);
    for (u32 i = 0; i < THUMB_BLOCK_CACHE_SIZE; i++)
    {
      thumb_block_cache[i].start_pc = 0xFFFFFFFF;
      thumb_block_cache[i].ram_page0 = -1;
      thumb_block_cache[i].ram_page1 = -1;
    }
  }

  memset(thumb_block_ram_page_generation, 0, sizeof(thumb_block_ram_page_generation));
  memset(thumb_block_ram_page_has_code, 0, sizeof(thumb_block_ram_page_has_code));
  thumb_block_ram_code_region_mask = 0;
#if THUMB_BLOCK_PROFILE
  memset(&thumb_block_stats, 0, sizeof(thumb_block_stats));
#endif
}

static bool thumb_block_init_cache(void)
{
  if (thumb_block_cache_ready)
    return thumb_block_cache != NULL;

  thumb_block_cache_ready = true;

#ifdef RETRO_GO
  thumb_block_cache = (thumb_block *)rg_alloc(sizeof(thumb_block) * THUMB_BLOCK_CACHE_SIZE, THUMB_BLOCK_CACHE_MEM);
#else
  thumb_block_cache = (thumb_block *)calloc(THUMB_BLOCK_CACHE_SIZE, sizeof(thumb_block));
#endif

  if (thumb_block_cache)
    thumb_block_flush_cache();

  return thumb_block_cache != NULL;
}

static inline void thumb_block_set_nz(u32 dest, u32 &n_flag, u32 &z_flag)
{
  z_flag = (dest == 0);
  n_flag = ((s32)dest < 0);
}

static inline void thumb_block_flags_add(u32 dest, u32 src_a, u32 src_b,
                                         u32 &n_flag, u32 &z_flag, u32 &v_flag)
{
  thumb_block_set_nz(dest, n_flag, z_flag);
  v_flag = ((~(src_a ^ src_b) & (src_a ^ dest)) >> 31);
}

static inline void thumb_block_flags_sub(u32 dest, u32 src_a, u32 src_b, u32 carry,
                                         u32 &n_flag, u32 &z_flag,
                                         u32 &c_flag, u32 &v_flag)
{
  thumb_block_set_nz(dest, n_flag, z_flag);
  c_flag = carry ? ((u32)src_b <= (u32)src_a) : ((u32)src_b < (u32)src_a);
  v_flag = (((src_a ^ src_b) & (~src_b ^ dest)) >> 31);
}

static inline void thumb_block_collapse_flags(u32 n_flag, u32 z_flag,
                                              u32 c_flag, u32 v_flag)
{
  reg[REG_CPSR] = (n_flag << 31) | (z_flag << 30) |
                  (c_flag << 29) | (v_flag << 28) |
                  (reg[REG_CPSR] & 0xFF);
}

static inline void thumb_block_fetch_cycles(u32 pc, s32 &cycles_remaining)
{
  u32 region = pc >> 24;
  if (region == 0x08 || region == 0x09)
    cycles_remaining -= 2;  /* ROM is cached in RAM in this port */
  else
    cycles_remaining -= ws_cyc_seq[region & 0xF][0];
}

static inline void thumb_block_nonseq_cycles(u32 pc, s32 &cycles_remaining)
{
  cycles_remaining -= ws_cyc_nseq[(pc >> 24) & 0x0F][0];
}

static inline u32 thumb_block_read8(u32 address, s32 &cycles_remaining)
{
  u8 *map;
  ...
  if (address >= 0x10000000 ||
      ((address >> 24) == 0 && reg[REG_PC] >= 0x4000) ||
      (address & aligned_address_mask8) ||
...
static inline u32 thumb_block_read16(u32 address, s32 &cycles_remaining)
{
  u8 *map;
  if (address < 0x10000000)
  {
  ...
  if (address >= 0x10000000 ||
      ((address >> 24) == 0 && reg[REG_PC] >= 0x4000) ||
      (address & aligned_address_mask16) ||
      !(map = memory_map_read[address >> 15]))
...
static inline u32 thumb_block_read16s(u32 address, s32 &cycles_remaining)
{
  u8 *map;
  if (address < 0x10000000)
  {
...
static inline u32 thumb_block_read32(u32 address, s32 &cycles_remaining)
{
  u8 *map;
  if (address < 0x10000000)
  {
  ...
  if (address >= 0x10000000 ||
      ((address >> 24) == 0 && reg[REG_PC] >= 0x4000) ||
      (address & aligned_address_mask32) ||
      !(map = memory_map_read[address >> 15]))
...
static inline void thumb_block_write8(u32 address, u32 value,
                                      ...
                                      cpu_alert_type &cpu_alert)
{
  ...
  cpu_alert |= write_memory8(address, (u8)value);
}

static inline void thumb_block_write16(u32 address, u32 value,
                                       s32 &cycles_remaining,
                                       cpu_alert_type &cpu_alert)
{
  address &= ~1U;
  ...
  cpu_alert |= write_memory16(address, (u16)value);
}

static inline void thumb_block_write32(u32 address, u32 value,
                                       s32 &cycles_remaining,
                                       cpu_alert_type &cpu_alert)
{
  address &= ~3U;
...
static inline bool thumb_block_pc_cacheable(u32 pc)
{
  switch (pc >> 24)
  {
    case 0x00:
      /* BIOS is executable only in its real 16 KB window. Reads above this
       * area are open-bus/BIOS-protected and should stay on the original
       * interpreter path. */
      return pc < 0x4000;
    case 0x02:  /* EWRAM */
    case 0x03:  /* IWRAM */
      return true;
    case 0x08:  /* GamePak ROM waitstate 0 */
    case 0x09:
    case 0x0A:  /* GamePak ROM waitstate 1 */
    case 0x0B:
    case 0x0C:  /* GamePak ROM waitstate 2 */
    case 0x0D:
      /* Stay on the original interpreter for open-bus reads beyond the
       * loaded ROM image. A Thumb opcode fetch needs two bytes, so the
       * final byte of an odd-sized image is not cacheable. */
      {
        const u32 offset = pc & 0x1FFFFFF;
        return offset < gamepak_size && (offset + 1) < gamepak_size;
      }
    default:
      return false;
  }
...
static u16 thumb_block_fetch_opcode(u32 pc)
{
  u8 *map = memory_map_read[pc >> 15];
  if (!map && (pc >> 24) >= 0x08 && (pc >> 24) <= 0x0D)
    map = load_gamepak_page((pc >> 15) & 0x3FF);

  if (map)
    return readaddress16(map, pc & 0x7FFF);

  return (u16)read_memory16(pc);
}

static bool thumb_block_decode_op(u16 opcode, thumb_block_op &op, bool &terminal)
{
  ...
  if (hi <= 0x17)
  {
    op.rd = opcode & 7;
    op.rs = (opcode >> 3) & 7;
    op.simm = (opcode >> 6) & 0x1F;
    op.handler = (hi < 0x08) ? TBH_LSL_IMM : (hi < 0x10) ? TBH_LSR_IMM : TBH_ASR_IMM;
    return true;
  }
  ...
  if (hi <= 0x1F)
  {
    ...
    switch ((opcode >> 9) & 3)
    {
      case 0: op.handler = TBH_ADD_REG; break;
      case 1: op.handler = TBH_SUB_REG; break;
      case 2: op.handler = TBH_ADD_IMM3; op.simm = op.rn; break;
      default: op.handler = TBH_SUB_IMM3; op.simm = op.rn; break;
    }
    return true;
  }
  ...
  if (hi <= 0x3F)
  {
    ...
    switch ((hi >> 3) & 3)
    {
      case 0: op.handler = TBH_MOV_IMM; break;
      case 1: op.handler = TBH_CMP_IMM; break;
      case 2: op.handler = TBH_ADD_IMM8; break;
      default: op.handler = TBH_SUB_IMM8; break;
    }
    return true;
  }
  ...
  if (hi >= 0x40 && hi <= 0x43)
  {
    static const u8 alu_handlers[16] =
    {
      TBH_AND_REG, TBH_EOR_REG, TBH_LSL_REG, TBH_LSR_REG,
      TBH_ASR_REG, TBH_ADC_REG, TBH_SBC_REG, TBH_ROR_REG,
      TBH_TST_REG, TBH_NEG_REG, TBH_CMP_REG, TBH_CMN_REG,
      TBH_ORR_REG, TBH_MUL_REG, TBH_BIC_REG, TBH_MVN_REG
    };
    op.rd = opcode & 7;
    op.rs = (opcode >> 3) & 7;
    op.handler = alu_handlers[(opcode >> 6) & 0x0F];
    return true;
  }

  if (hi >= 0x44 && hi <= 0x47)
  {
    op.rd = ((opcode >> 4) & 0x08) | (opcode & 0x07);
    op.rs = (opcode >> 3) & 0x0F;
    switch (hi)
    {
      case 0x44:
        op.handler = TBH_HI_ADD;
        terminal = (op.rd == REG_PC);
        return true;
      case 0x45:
        op.handler = TBH_HI_CMP;
        return true;
      case 0x46:
        op.handler = TBH_HI_MOV;
        terminal = (op.rd == REG_PC);
        return true;
      default:
        op.handler = TBH_BX;
  ...
  if (hi >= 0x50 && hi <= 0x5F)
  {
    static const u8 mem_reg_handlers[8] =
    {
      TBH_STR_REG, TBH_STRH_REG, TBH_STRB_REG, TBH_LDSB_REG,
      TBH_LDR_REG, TBH_LDRH_REG, TBH_LDRB_REG, TBH_LDSH_REG
    };
    op.rd = opcode & 7;
    op.rn = (opcode >> 3) & 7;
    op.rs = (opcode >> 6) & 7;
    op.handler = mem_reg_handlers[(opcode >> 9) & 7];
    return true;
  }
  ...
  if (hi >= 0x60 && hi <= 0x8F)
  {
    ...
    op.rn = (opcode >> 3) & 7;
    op.simm = (opcode >> 6) & 0x1F;
    if (hi < 0x68) op.handler = TBH_STR_IMM;
    else if (hi < 0x70) op.handler = TBH_LDR_IMM;
    else if (hi < 0x78) op.handler = TBH_STRB_IMM;
    else if (hi < 0x80) op.handler = TBH_LDRB_IMM;
    else if (hi < 0x88) op.handler = TBH_STRH_IMM;
    else op.handler = TBH_LDRH_IMM;
    return true;
  }
  ...
  if (hi >= 0x90 && hi <= 0x9F)
  {
    op.rd = hi & 7;
    op.simm = opcode & 0xFF;
    op.handler = (hi < 0x98) ? TBH_STR_SP : TBH_LDR_SP;
    return true;
  }
  ...
  if (hi >= 0xA0 && hi <= 0xAF)
  {
    op.rd = hi & 7;
    op.simm = opcode & 0xFF;
    op.handler = (hi < 0xA8) ? TBH_ADD_PC : TBH_ADD_SP;
    return true;
  }

  if (hi >= 0xB0 && hi <= 0xB3)
  {
    op.handler = TBH_ADD_SP_IMM;
    op.simm = opcode & 0x7F;
    return true;
  }
  ...
  if (hi >= 0xC0 && hi <= 0xCF)
  {
    op.rn = hi & 7;
    op.simm = opcode & 0xFF;
    op.handler = (hi < 0xC8) ? TBH_STMIA : TBH_LDMIA;
    return true;
  }

  if (hi >= 0xD0 && hi <= 0xDD)
  {
    op.handler = TBH_B_COND;
    op.simm = (s8)(opcode & 0xFF);
    /* Keep decoding fall-through instructions. A taken branch exits the block;
     * a not-taken branch can continue without re-entering the cache lookup. */
    return true;
  }
  ...
  if (hi >= 0xE0 && hi <= 0xE7)
  {
    ...
    op.handler = TBH_B_UNCOND;
    op.simm = (s32)(offset << 21) >> 20;
    terminal = true;
    return true;
  }
  ...
  if (hi >= 0xF0 && hi <= 0xF7)
  {
    /* Keep the full signed BL prefix calculation in the handler. The
     * displacement is wider than thumb_block_op::simm, so storing it here
     * would silently truncate long branches. */
    op.handler = TBH_BL_LOW;
    return true;
  }

  if (hi >= 0xF8)
  {
    op.handler = TBH_BL_HIGH;
    op.simm = opcode & 0x7FF;
    terminal = true;
    return true;
  }
...
static inline u32 thumb_block_cache_index(u32 pc)
{
  /* Low-bit-only direct mapping is prone to thrashing when hot functions are
   * spaced by powers of two. Mix in higher address bits, while keeping the
   * table power-of-two and cheap to index on ESP32-S3. */
  return ((pc >> 1) ^ (pc >> 11) ^ (pc >> 19)) & THUMB_BLOCK_CACHE_MASK;
}

static inline void thumb_block_bind_ram_pages(thumb_block *block, u32 start_pc, u32 end_pc)
{
  const s16 page0 = thumb_block_ram_page_index(start_pc);
  const s16 page1 = thumb_block_ram_page_index(end_pc);

  if (page0 >= 0)
  {
    block->ram_page0 = page0;
    block->ram_generation0 = thumb_block_ram_page_generation[page0];
    thumb_block_mark_ram_page(page0);

    if (page1 >= 0 && page1 != page0)
    {
      block->ram_page1 = page1;
      block->ram_generation1 = thumb_block_ram_page_generation[page1];
      thumb_block_mark_ram_page(page1);
    }
  }
}

static thumb_block *thumb_block_build(u32 pc)
{
  if (!thumb_block_init_cache())
    return NULL;

  thumb_block *block = &thumb_block_cache[thumb_block_cache_index(pc)];
  const u32 start_region = pc >> 15;
  block->start_pc = pc;
  block->region = start_region;
  block->ram_page0 = -1;
  block->ram_page1 = -1;
  block->ram_generation0 = 0;
  block->ram_generation1 = 0;
  block->op_count = 0;
  block->terminal = 0;

  for (u32 i = 0; i < THUMB_BLOCK_MAX_OPS; i++)
  {
    u32 op_pc = pc + (i * 2);

    if ((op_pc >> 15) != start_region || !thumb_block_pc_cacheable(op_pc))
      break;

    bool terminal = false;
    thumb_block_op op;
    if (!thumb_block_decode_op(thumb_block_fetch_opcode(op_pc), op, terminal))
    {
      TB_STAT_INC(unsupported_ops);
      break;
    }
  ...
  if (block->op_count)
  {
    const u32 block_end_pc = pc + ((u32)block->op_count * 2) - 1;
    thumb_block_bind_ram_pages(block, pc, block_end_pc);
  }
  else
  {
    /* Unsupported RAM opcodes still need a generation tag. Otherwise a game
     * could patch an unsupported instruction into a supported one and the
     * zero-op cache entry would suppress future rebuilds forever. */
    thumb_block_bind_ram_pages(block, pc, pc + 1);
  }

  TB_STAT_INC(blocks_built);
  TB_STAT_ADD(decoded_ops, block->op_count);
  return block;
}

static inline bool thumb_block_condition_true(u32 cond, u32 n_flag, u32 z_flag,
                                              u32 c_flag, u32 v_flag)
{
  switch (cond)
  {
    case 0x0: return z_flag == 1;
    case 0x1: return z_flag == 0;
    case 0x2: return c_flag == 1;
    case 0x3: return c_flag == 0;
    case 0x4: return n_flag == 1;
    case 0x5: return n_flag == 0;
    case 0x6: return v_flag == 1;
    case 0x7: return v_flag == 0;
    case 0x8: return c_flag & (z_flag ^ 1);
    case 0x9: return (c_flag == 0) | z_flag;
    case 0xA: return n_flag == v_flag;
    case 0xB: return n_flag != v_flag;
    case 0xC: return (z_flag == 0) & (n_flag == v_flag);
    case 0xD: return z_flag | (n_flag != v_flag);
    default: return false;
  }
}

static inline bool thumb_block_ram_generation_changed(const thumb_block *block);

static inline u32 thumb_block_ror_reg(u32 dest, u32 shift, u32 &c_flag)
{
  shift &= 0xFF;
  if (shift == 0)
    return dest;
...
static int IRAM_ATTR thumb_block_run(thumb_block *block,
                                     u32 &n_flag, u32 &z_flag,
                                     u32 &c_flag, u32 &v_flag,
                                     s32 &cycles_remaining,
                                     cpu_alert_type &cpu_alert)
{
  thumb_block_op *op = block->ops;
  thumb_block_op *end = block->ops + block->op_count;

  static const void *dispatch[TBH_COUNT] =
  {
    &&h_lsl_imm,
    &&h_lsr_imm,
    &&h_asr_imm,
  ...
  u32 pc = block->start_pc;

#define TB_NEXT_COMMON(check_smc)                                             \
  do {                                                                        \
    reg[REG_PC] = pc + 2;                                                     \
    thumb_block_fetch_cycles(reg[REG_PC], cycles_remaining);                  \
    pc = reg[REG_PC];                                                         \
    if ((check_smc) && thumb_block_ram_generation_changed(block))             \
      return THUMB_BLOCK_EXIT_RAN;                                            \
    if (reg[REG_PC] == idle_loop_target_pc && cycles_remaining > 0)           \
      cycles_remaining = 0;                                                   \
    if (cpu_alert & (CPU_ALERT_HALT | CPU_ALERT_IRQ | CPU_ALERT_SMC))         \
      return THUMB_BLOCK_EXIT_RAN;                                            \
    if (++op >= end || cycles_remaining <= 0)                                 \
      return THUMB_BLOCK_EXIT_RAN;                                            \
    goto *dispatch[op->handler];                                              \
  } while (0)

#define TB_NEXT() TB_NEXT_COMMON(false)
#define TB_NEXT_AFTER_WRITE() TB_NEXT_COMMON(true)

#define TB_BRANCH_DONE()                                                      \
  do {                                                                        \
    thumb_block_fetch_cycles(reg[REG_PC], cycles_remaining);                  \
    if (reg[REG_PC] == idle_loop_target_pc && cycles_remaining > 0)           \
      cycles_remaining = 0;                                                   \
...
#define TB_ADD_FLAGS(dest, a, b, c_in)                                        \
  do {                                                                        \
    const u32 _sa = (a);                                                       \
    const u32 _sb = (b);                                                       \
    const u32 _sc = (c_in);                                                    \
    ...
    _dest += _sc;                                                             \
    c_flag |= (_dest < _sc);                                                   \
    thumb_block_flags_add(_dest, _sa, _sb, n_flag, z_flag, v_flag);           \
    dest = _dest;                                                             \
  } while (0)

#define TB_SUB_FLAGS(dest, a, b, c_in)                                        \
  do {                                                                        \
    const u32 _sa = (a);                                                       \
    const u32 _sb = (b);                                                       \
    const u32 _sc = (c_in);                                                    \
    u32 _dest = _sa + (~_sb) + _sc;                                            \
    thumb_block_flags_sub(_dest, _sa, _sb, _sc, n_flag, z_flag, c_flag, v_flag);\
    dest = _dest;                                                             \
  } while (0)

#define TB_IMM ((u32)(u16)op->simm)
#define TB_REGLIST ((u16)op->simm)

  if (op >= end)
    return THUMB_BLOCK_EXIT_NONE;
...
h_add_reg:
  TB_ADD_FLAGS(reg[op->rd], reg[op->rs], reg[op->rn], 0);
  TB_NEXT();

h_sub_reg:
  TB_SUB_FLAGS(reg[op->rd], reg[op->rs], reg[op->rn], 1);
  TB_NEXT();

h_add_imm3:
  TB_ADD_FLAGS(reg[op->rd], reg[op->rs], TB_IMM, 0);
  TB_NEXT();

h_sub_imm3:
  TB_SUB_FLAGS(reg[op->rd], reg[op->rs], TB_IMM, 1);
  TB_NEXT();

h_mov_imm:
  reg[op->rd] = TB_IMM;
  thumb_block_set_nz(reg[op->rd], n_flag, z_flag);
  TB_NEXT();

h_cmp_imm:
  {
    u32 dest = reg[op->rd] - TB_IMM;
    thumb_block_flags_sub(dest, reg[op->rd], TB_IMM, 1, n_flag, z_flag, c_flag, v_flag);
    TB_NEXT();
  }

h_add_imm8:
  TB_ADD_FLAGS(reg[op->rd], reg[op->rd], TB_IMM, 0);
  TB_NEXT();

h_sub_imm8:
  TB_SUB_FLAGS(reg[op->rd], reg[op->rd], TB_IMM, 1);
  TB_NEXT();

h_and_reg:
  reg[op->rd] = reg[op->rd] & reg[op->rs];
  thumb_block_set_nz(reg[op->rd], n_flag, z_flag);
  TB_NEXT();
...
h_lsl_reg:
  {
    u32 shift = reg[op->rs] & 0xFF;
    u32 dest = reg[op->rd];
    if (shift != 0)
    {
      if (shift > 31)
      {
        c_flag = (shift == 32) ? (dest & 1) : 0;
        dest = 0;
      }
      else
      {
        c_flag = (dest >> (32 - shift)) & 1;
        dest <<= shift;
      }
    }
    thumb_block_set_nz(dest, n_flag, z_flag);
...
h_lsr_reg:
  {
    ...
    if (shift != 0)
    {
      if (shift > 31)
      {
        c_flag = (shift == 32) ? (dest >> 31) : 0;
        dest = 0;
      }
      else
      {
        c_flag = (dest >> (shift - 1)) & 1;
        dest >>= shift;
      }
    }
    thumb_block_set_nz(dest, n_flag, z_flag);
...
h_adc_reg:
  TB_ADD_FLAGS(reg[op->rd], reg[op->rd], reg[op->rs], c_flag);
  TB_NEXT();

h_sbc_reg:
  TB_SUB_FLAGS(reg[op->rd], reg[op->rd], reg[op->rs], c_flag);
  TB_NEXT();

h_ror_reg:
  {
    u32 dest = thumb_block_ror_reg(reg[op->rd], reg[op->rs], c_flag);
    thumb_block_set_nz(dest, n_flag, z_flag);
    reg[op->rd] = dest;
    TB_NEXT();
  }
...
h_tst_reg:
  thumb_block_set_nz(reg[op->rd] & reg[op->rs], n_flag, z_flag);
  TB_NEXT();

h_neg_reg:
  TB_SUB_FLAGS(reg[op->rd], 0, reg[op->rs], 1);
  TB_NEXT();

h_cmp_reg:
  {
    u32 dest = reg[op->rd] - reg[op->rs];
    thumb_block_flags_sub(dest, reg[op->rd], reg[op->rs], 1, n_flag, z_flag, c_flag, v_flag);
    TB_NEXT();
  }
...
h_cmn_reg:
  {
    u32 dest = reg[op->rd] + reg[op->rs];
    c_flag = (dest < reg[op->rs]);
    thumb_block_flags_add(dest, reg[op->rd], reg[op->rs], n_flag, z_flag, v_flag);
    TB_NEXT();
  }
...
h_hi_add:
  {
    u32 lhs = (op->rd == REG_PC) ? (pc + 4) : reg[op->rd];
    u32 rhs = (op->rs == REG_PC) ? (pc + 4) : reg[op->rs];
    u32 dest = lhs + rhs;
    if (op->rd == REG_PC)
    {
      reg[REG_PC] = dest & ~1U;
...
h_hi_cmp:
  {
    ...
    u32 rhs = (op->rs == REG_PC) ? (pc + 4) : reg[op->rs];
    u32 dest = lhs - rhs;
    thumb_block_flags_sub(dest, lhs, rhs, 1, n_flag, z_flag, c_flag, v_flag);
    TB_NEXT();
  }

h_hi_mov:
  {
    u32 dest = (op->rs == REG_PC) ? (pc + 4) : reg[op->rs];
    if (op->rd == REG_PC)
    {
      reg[REG_PC] = dest & ~1U;
      TB_BRANCH_DONE();
...
h_bx:
  {
    u32 src = (op->rs == REG_PC) ? (pc + 4) : reg[op->rs];
    if (src & 1)
    {
      reg[REG_PC] = src - 1;
      TB_BRANCH_DONE();
    ...
    {
      reg[REG_PC] = src;
      thumb_block_collapse_flags(n_flag, z_flag, c_flag, v_flag);
      reg[REG_CPSR] &= ~0x20;
      return THUMB_BLOCK_EXIT_ARM;
    }
  }

h_ldr_literal:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read32((pc & ~2U) + 4 + (TB_IMM * 4), cycles_remaining);
  TB_NEXT();

h_str_reg:
  reg[REG_PC] = pc + 2;
  thumb_block_write32(reg[op->rn] + reg[op->rs], reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_strh_reg:
  reg[REG_PC] = pc + 2;
  thumb_block_write16(reg[op->rn] + reg[op->rs], reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_strb_reg:
  reg[REG_PC] = pc + 2;
  thumb_block_write8(reg[op->rn] + reg[op->rs], reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_ldsb_reg:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = (u32)((s8)thumb_block_read8(reg[op->rn] + reg[op->rs], cycles_remaining));
  TB_NEXT();

h_ldr_reg:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read32(reg[op->rn] + reg[op->rs], cycles_remaining);
  TB_NEXT();

h_ldrh_reg:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read16(reg[op->rn] + reg[op->rs], cycles_remaining);
  TB_NEXT();

h_ldrb_reg:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read8(reg[op->rn] + reg[op->rs], cycles_remaining);
  TB_NEXT();

h_ldsh_reg:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read16s(reg[op->rn] + reg[op->rs], cycles_remaining);
  TB_NEXT();

h_str_imm:
  reg[REG_PC] = pc + 2;
  thumb_block_write32(reg[op->rn] + (TB_IMM * 4), reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_ldr_imm:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read32(reg[op->rn] + (TB_IMM * 4), cycles_remaining);
  TB_NEXT();

h_strb_imm:
  reg[REG_PC] = pc + 2;
  thumb_block_write8(reg[op->rn] + TB_IMM, reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_ldrb_imm:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read8(reg[op->rn] + TB_IMM, cycles_remaining);
  TB_NEXT();

h_strh_imm:
  reg[REG_PC] = pc + 2;
  thumb_block_write16(reg[op->rn] + (TB_IMM * 2), reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_ldrh_imm:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read16(reg[op->rn] + (TB_IMM * 2), cycles_remaining);
  TB_NEXT();

h_str_sp:
  reg[REG_PC] = pc + 2;
  thumb_block_write32(reg[REG_SP] + (TB_IMM * 4), reg[op->rd], cycles_remaining, cpu_alert);
  TB_NEXT_AFTER_WRITE();

h_ldr_sp:
  reg[REG_PC] = pc + 2;
  reg[op->rd] = thumb_block_read32(reg[REG_SP] + (TB_IMM * 4), cycles_remaining);
  TB_NEXT();

h_add_pc:
  reg[op->rd] = (pc & ~2U) + 4 + (TB_IMM * 4);
  TB_NEXT();

h_add_sp:
  reg[op->rd] = reg[REG_SP] + (TB_IMM * 4);
  TB_NEXT();
...
h_push:
  ...
  cpu_alert |= exec_thumb_block_mem<AccStore, AddrPreDec>(
    REG_SP, TB_REGLIST, cycles_remaining);
  TB_NEXT_AFTER_WRITE();
...
h_stmia:
  ...
  cpu_alert |= exec_thumb_block_mem<AccStore, AddrPostInc>(
    op->rn, TB_REGLIST, cycles_remaining);
  TB_NEXT_AFTER_WRITE();
...
h_b_cond:
  if (thumb_block_condition_true((op->opcode >> 8) & 0x0F, n_flag, z_flag, c_flag, v_flag))
  {
    reg[REG_PC] = pc + ((s32)op->simm * 2) + 4;
    thumb_block_nonseq_cycles(reg[REG_PC], cycles_remaining);
    TB_BRANCH_DONE();
  }
  thumb_block_nonseq_cycles(pc + 2, cycles_remaining);
  TB_NEXT();

h_b_uncond:
  reg[REG_PC] = pc + op->simm + 4;
  thumb_block_nonseq_cycles(reg[REG_PC], cycles_remaining);
  TB_BRANCH_DONE();

h_bl_low:
  {
    const u32 offset = op->opcode & 0x07FF;
    const s32 signed_offset = (s32)(offset << 21) >> 9;
    reg[REG_LR] = pc + 4 + signed_offset;
    TB_NEXT();
  }
...
h_bl_high:
  {
    ...
    reg[REG_LR] = newlr;
    reg[REG_PC] = newpc;
    thumb_block_nonseq_cycles(newpc, cycles_remaining);
    TB_BRANCH_DONE();
  }
...
#undef TB_NEXT_AFTER_WRITE
#undef TB_NEXT
#undef TB_NEXT_COMMON
#undef TB_BRANCH_DONE
#undef TB_ADD_FLAGS
#undef TB_SUB_FLAGS
#undef TB_IMM
#undef TB_REGLIST
...
static inline bool thumb_block_ram_generation_changed(const thumb_block *block)
{
  return (block->ram_page0 >= 0 &&
          block->ram_generation0 != thumb_block_ram_page_generation[block->ram_page0]) ||
         (block->ram_page1 >= 0 &&
          block->ram_generation1 != thumb_block_ram_page_generation[block->ram_page1]);
}

static inline int thumb_block_try_run(u32 pc,
                                      u32 &n_flag, u32 &z_flag,
                                      u32 &c_flag, u32 &v_flag,
                                      s32 &cycles_remaining,
                                      cpu_alert_type &cpu_alert)
{
  ...
  if (cycles_remaining <= 0 || !thumb_block_pc_cacheable(pc) ||
      !thumb_block_init_cache())
    return THUMB_BLOCK_EXIT_NONE;

  /* CPU_ALERT_SMC is a dynarec/block-runner stop signal, not a condition that
   * the plain interpreter handles. If it leaks into the next chain attempt it
   * would make every block exit immediately after one instruction. */
  cpu_alert = (cpu_alert_type)(cpu_alert & ~CPU_ALERT_SMC);

  bool ran_any = false;

  for (u32 chain = 0; chain < THUMB_BLOCK_CHAIN_LIMIT; chain++)
  {
    pc = reg[REG_PC] & ~1U;

    /* Preserve the original interpreter's cheat hook semantics. If chaining
     * lands on the hook, return to execute_arm() so process_cheats() runs at
     * the top of the THUMB loop before that instruction is executed. */
    if (ran_any && pc == cheat_master_hook)
      return THUMB_BLOCK_EXIT_RAN;

    if (!thumb_block_pc_cacheable(pc))
      return ran_any ? THUMB_BLOCK_EXIT_RAN : THUMB_BLOCK_EXIT_NONE;

    thumb_block *block = &thumb_block_cache[thumb_block_cache_index(pc)];
    const u32 region = pc >> 15;

    if (block->start_pc != pc || block->region != region ||
        thumb_block_ram_generation_changed(block))
    {
      TB_STAT_INC(cache_misses);
      block = thumb_block_build(pc);
    }
    else
    {
      TB_STAT_INC(cache_hits);
    }

    if (!block || block->op_count == 0)
      return ran_any ? THUMB_BLOCK_EXIT_RAN : THUMB_BLOCK_EXIT_NONE;

    TB_STAT_INC(blocks_run);
    int exit_type = thumb_block_run(block, n_flag, z_flag, c_flag, v_flag,
                                    cycles_remaining, cpu_alert);

    if (exit_type == THUMB_BLOCK_EXIT_ARM)
      return THUMB_BLOCK_EXIT_ARM;

    if (exit_type != THUMB_BLOCK_EXIT_RAN)
      return ran_any ? THUMB_BLOCK_EXIT_RAN : exit_type;

    ran_any = true;

    if (cpu_alert & CPU_ALERT_SMC)
    {
      cpu_alert = (cpu_alert_type)(cpu_alert & ~CPU_ALERT_SMC);
      return THUMB_BLOCK_EXIT_RAN;
    }
    ...
    if (cycles_remaining <= 0 ||
        (cpu_alert & (CPU_ALERT_HALT | CPU_ALERT_IRQ)))
      return THUMB_BLOCK_EXIT_RAN;
  }
...
arm_loop:
       ...
       /* ICACHE: cache opcodes to skip PSRAM reads in tight loops */
       {
         #define ICACHE_SIZE 256
         static u32 ic_pc[ICACHE_SIZE], ic_op[ICACHE_SIZE];
         u32 idx = (reg[REG_PC] >> 2) & (ICACHE_SIZE - 1);
         if (ic_pc[idx] == reg[REG_PC])
           opcode = ic_op[idx];
         else {
           opcode = readaddress32(pc_address_block, (reg[REG_PC] & 0x7FFF));
           ic_pc[idx] = reg[REG_PC];
           ic_op[idx] = opcode;
         }
       }
       condition = opcode >> 28;
...
skip_instruction:
       ...
       {
         u32 region = reg[REG_PC] >> 24;
         if (region == 0x08 || region == 0x09)
           cycles_remaining -= 4;  // Fast path: ROM cached in RAM, cap at 2+2
         else
           cycles_remaining -= ws_cyc_seq[region & 0xF][1];
       }
...
thumb_loop:
...
#if ENABLE_THUMB_BLOCK_INTERPRETER
       {
          int thumb_block_exit = thumb_block_try_run(reg[REG_PC],
             n_flag, z_flag, c_flag, v_flag, cycles_remaining, cpu_alert);

          if (thumb_block_exit == THUMB_BLOCK_EXIT_ARM)
             goto arm_loop;

          if (thumb_block_exit == THUMB_BLOCK_EXIT_RAN)
          {
             check_pc_region();
       ...
       /* End of Execute THUMB instruction */
       {
         u32 region = reg[REG_PC] >> 24;
         if (region == 0x08 || region == 0x09)
           cycles_remaining -= 2;  // Fast path: ROM cached in RAM
         else
           cycles_remaining -= ws_cyc_seq[region & 0xF][0];
       }
...
{
  const u8 *cpudoc = bson_find_key(src, "cpu");
  bool ok = bson_read_int32(cpudoc, "bus-value", &reg[REG_BUS_VALUE]) &&
