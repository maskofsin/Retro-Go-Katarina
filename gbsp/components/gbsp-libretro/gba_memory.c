...
// ROM memory is cached in 32KB pages, backed by larger malloc'ed slabs.
//
// The old code allocated ROM_BUFFER_SIZE slabs of 1MB each. That works when
// the heap is contiguous, but on ESP32-S3/PSRAM it is easy for later 1MB
// allocations to fail from fragmentation. Keep the same default total budget
// semantics, but make the slab size configurable and default it to 512KB.
//
// If the build passes -DROM_BUFFER_SIZE=4, the target budget remains 4MB, but
// it is now allocated as eight 512KB chunks. To try a larger cache for 16MB
// games, pass -DROM_BUFFER_SIZE=8 or -DGAMEPAK_BUFFER_TARGET_SIZE=(8*1024*1024).
#ifndef ROM_BUFFER_SIZE
#define ROM_BUFFER_SIZE 4
#endif

#ifndef GAMEPAK_ROM_PAGE_SIZE
#define GAMEPAK_ROM_PAGE_SIZE (32 * 1024)
#endif

#ifndef GAMEPAK_BUFFER_BLOCK_SIZE
#define GAMEPAK_BUFFER_BLOCK_SIZE (512 * 1024)
#endif

#ifndef GAMEPAK_BUFFER_TARGET_SIZE
#define GAMEPAK_BUFFER_TARGET_SIZE (ROM_BUFFER_SIZE * 1024 * 1024)
#endif

#ifndef GAMEPAK_PREFETCH_PAGES
#define GAMEPAK_PREFETCH_PAGES 4
#endif

// If the whole ROM fits in the allocated cache, load and map every page at
// startup and then forbid runtime SD reads. This turns small/medium ROMs into
// a true resident direct-map and makes stutter diagnosis much easier.
#ifndef GAMEPAK_RESIDENT_MODE
#define GAMEPAK_RESIDENT_MODE 1
#endif

// Optional ROM pager diagnostics. Keep disabled for normal play because printf
// during gameplay can itself cause stutter.
#ifndef GAMEPAK_PROFILE
#define GAMEPAK_PROFILE 0
#endif

#ifndef GAMEPAK_PROFILE_LOG_EVERY
#define GAMEPAK_PROFILE_LOG_EVERY 64
#endif

#if (GAMEPAK_BUFFER_BLOCK_SIZE % GAMEPAK_ROM_PAGE_SIZE) != 0
#error GAMEPAK_BUFFER_BLOCK_SIZE must be a multiple of GAMEPAK_ROM_PAGE_SIZE
#endif

#if GAMEPAK_PREFETCH_PAGES < 1
#error GAMEPAK_PREFETCH_PAGES must be at least 1
#endif

#define GAMEPAK_MAX_ROM_PAGES 1024
#define GAMEPAK_PAGES_PER_BUFFER_BLOCK \
  (GAMEPAK_BUFFER_BLOCK_SIZE / GAMEPAK_ROM_PAGE_SIZE)

#if GAMEPAK_PAGES_PER_BUFFER_BLOCK == 0
#error GAMEPAK_BUFFER_BLOCK_SIZE is too small
#endif

// Worst case: one malloc'ed slab per 32KB ROM page. This costs only a few KB
// for the pointer table and keeps smaller experimental block sizes possible.
u8 *gamepak_buffers[GAMEPAK_MAX_ROM_PAGES];
u32 gamepak_buffer_count;        /* Number of malloc'ed backing slabs */
u32 gamepak_buffer_entry_count;  /* Number of cached 32KB ROM pages */
u32 gamepak_size;                /* Size of the ROM in bytes, rounded to 32KB */
u32 gamepak_raw_size;            /* Exact ROM file size before 32KB rounding */
u32 gamepak_rom_pages;           /* gamepak_size / 32KB */
bool gamepak_rom_resident;        /* True when every ROM page is cached/mapped */
u32 gamepak_initial_pages_loaded; /* Pages loaded during load_gamepak_raw() */
const unsigned gamepak_buffer_blocksize = GAMEPAK_BUFFER_BLOCK_SIZE;

#if GAMEPAK_PROFILE
u32 gamepak_runtime_page_faults;
u32 gamepak_runtime_file_reads;
u32 gamepak_runtime_prefetch_reads;
u32 gamepak_runtime_resident_misses;
u32 gamepak_runtime_read_kb;
#endif

// LRU queue with the loaded 32KB pages and what they map to.
struct {
  u16 next_lru;             /* Index in the struct to the next LRU entry */
  s16 phy_rom;              /* ROM page number (-1 means not mapped) */
} gamepak_blk_queue[GAMEPAK_MAX_ROM_PAGES];

// Inverse map used by prefetch/loading to avoid loading the same physical page
// twice. -1 means the ROM page is not resident in the cache.
s16 gamepak_phy_to_entry[GAMEPAK_MAX_ROM_PAGES];
...
u32 gamepak_sticky_bit[GAMEPAK_MAX_ROM_PAGES/32];
...
#if 1 // Speed hack: ROM is in fast PSRAM, reduce wait states
  // Cap ROM wait states at 2 cycles per 16-bit access to reduce interpreter load
  if (ws_cyc_seq[0x8][0] > 2) ws_cyc_seq[0x8][0] = 2;
  if (ws_cyc_seq[0xA][0] > 2) ws_cyc_seq[0xA][0] = 2;
  if (ws_cyc_seq[0xC][0] > 2) ws_cyc_seq[0xC][0] = 2;
  ws_cyc_seq[0x9][0] = ws_cyc_seq[0x8][0];
  ws_cyc_seq[0xB][0] = ws_cyc_seq[0xA][0];
  ws_cyc_seq[0xD][0] = ws_cyc_seq[0xC][0];
#endif
...
#if 1 // Speed hack: cap non-sequential ROM wait states too
  if (ws_cyc_nseq[0x8][0] > 3) ws_cyc_nseq[0x8][0] = 3;
  if (ws_cyc_nseq[0xA][0] > 3) ws_cyc_nseq[0xA][0] = 3;
  if (ws_cyc_nseq[0xC][0] > 3) ws_cyc_nseq[0xC][0] = 3;
  ws_cyc_nseq[0x9][0] = ws_cyc_nseq[0x8][0];
  ws_cyc_nseq[0xB][0] = ws_cyc_nseq[0xA][0];
  ws_cyc_nseq[0xD][0] = ws_cyc_nseq[0xC][0];
#endif
...
#define write_memory(type)                                                    \
  ...
  {                                                                           \
    case 0x02:                                                                \
      ...
      thumb_block_invalidate_range_if_cached(address, (type / 8));                      \
      break;                                                                  \
                                                                              \
    case 0x03:                                                                \
      /* internal work RAM */                                                 \
...
static inline void thumb_block_invalidate_dma_dest_if_cached(
  u32 dest_ptr, int dest_stride, u32 length, u32 bytes_per_transfer)
{
  if (!length)
    return;

  u32 start = dest_ptr;
  u32 bytes = bytes_per_transfer;

  if (dest_stride > 0)
  {
    bytes += (length - 1) * (u32)dest_stride;
  }
  else if (dest_stride < 0)
  {
    u32 backwards = (length - 1) * (u32)(-dest_stride);
    start -= backwards;
    bytes += backwards;
  }

  thumb_block_invalidate_range_if_cached(start, bytes);
}
  ...
  dma_transfer_type *dmach, u32 src_ptr, u32 dest_ptr, u32 length)
{
  ...
  if (dmach->source_direction < 3)
  {
    ...
    if(dmach->length_type == DMA_16BIT)
    {
       ...
       thumb_block_invalidate_dma_dest_if_cached(dest_ptr, 2 * dst_stride, length, 2);
    }
    else
    {
       ret = dma_tf_loop32(src_ptr, dest_ptr, 4 * src_stride, 4 * dst_stride, dst_wb, length, dmach);
       thumb_block_invalidate_dma_dest_if_cached(dest_ptr, 4 * dst_stride, length, 4);
    }
  }
...
static inline u8 *gamepak_entry_ptr(u32 entry)
{
  u32 block_idx = entry / GAMEPAK_PAGES_PER_BUFFER_BLOCK;
  u32 block_off = entry % GAMEPAK_PAGES_PER_BUFFER_BLOCK;
  return &gamepak_buffers[block_idx][GAMEPAK_ROM_PAGE_SIZE * block_off];
}
...
static u32 evict_gamepak_page(void)
{
  ...
  do {
    ...
    gamepak_blk_queue[gamepak_lru_tail].next_lru = (u16)ret;
    gamepak_lru_tail = (u16)ret;

    // If every resident page became sticky in this frame, do not spin
    // forever. Clear the frame-local sticky set and evict the current page.
    if (++scanned > gamepak_buffer_entry_count)
    {
  ...
  if (phyrom >= 0) {
    ...
    if ((u32)phyrom < GAMEPAK_MAX_ROM_PAGES)
      gamepak_phy_to_entry[phyrom] = -1;
  }

  gamepak_blk_queue[ret].phy_rom = -1;
  return ret;
}

static void gamepak_read_rom_page(u32 physical_index, u8 *dest)
{
  u32 file_offset = physical_index * GAMEPAK_ROM_PAGE_SIZE;
  u32 bytes_left = file_offset < gamepak_raw_size ?
    gamepak_raw_size - file_offset : 0;
  u32 bytes_to_read = bytes_left < GAMEPAK_ROM_PAGE_SIZE ?
    bytes_left : GAMEPAK_ROM_PAGE_SIZE;

  memset(dest, 0xFF, GAMEPAK_ROM_PAGE_SIZE);
  if (bytes_to_read && gamepak_file_large)
  {
    fseek(gamepak_file_large, file_offset, SEEK_SET);
    fread(dest, 1, bytes_to_read, gamepak_file_large);
  }
}

static void gamepak_note_runtime_read(u32 physical_index, bool prefetch)
{
#if GAMEPAK_PROFILE
  gamepak_runtime_page_faults++;
  gamepak_runtime_file_reads++;
  gamepak_runtime_read_kb += GAMEPAK_ROM_PAGE_SIZE / 1024;
  if (prefetch)
    gamepak_runtime_prefetch_reads++;

  if ((gamepak_runtime_file_reads % GAMEPAK_PROFILE_LOG_EVERY) == 0)
  {
    printf("GBA ROM pager: %u SD reads, %uKB, last page %u, prefetch %u, resident miss %u\n",
      gamepak_runtime_file_reads, gamepak_runtime_read_kb, physical_index,
      gamepak_runtime_prefetch_reads, gamepak_runtime_resident_misses);
  }
#else
  (void)physical_index;
  (void)prefetch;
...
static u8 *load_gamepak_page_single(u32 physical_index)
{
  u32 rom_pages = gamepak_rom_pages;

  if(physical_index >= rom_pages)
    return gamepak_buffer_count ? &gamepak_buffers[0][0] : NULL;

  if (physical_index < GAMEPAK_MAX_ROM_PAGES && gamepak_phy_to_entry[physical_index] >= 0)
    return gamepak_entry_ptr((u32)gamepak_phy_to_entry[physical_index]);

#if GAMEPAK_RESIDENT_MODE
  // In resident mode every valid ROM page should already be mapped. If this
  // happens, do not silently hit the SD card during gameplay: report it in
  // profile builds and return a safe pointer rather than causing a crash.
  if (gamepak_rom_resident)
  {
#if GAMEPAK_PROFILE
    gamepak_runtime_resident_misses++;
    printf("GBA ROM resident miss: page %u/%u was not mapped\n",
      physical_index, rom_pages);
#endif
    return gamepak_buffer_count ? &gamepak_buffers[0][0] : NULL;
  }
#endif
  ...
  gamepak_note_runtime_read(physical_index, false);

  u32 entry = evict_gamepak_page();
  u8 *swap_location = gamepak_entry_ptr(entry);

  // Fill in the entry
  gamepak_blk_queue[entry].phy_rom = (s16)physical_index;
  if (physical_index < GAMEPAK_MAX_ROM_PAGES)
    gamepak_phy_to_entry[physical_index] = (s16)entry;

  gamepak_read_rom_page(physical_index, swap_location);

  // Map it to the read handlers now
  map_rom_entry(read, physical_index, swap_location, rom_pages);
...
static u8 *load_gamepak_page_prefetch(u32 physical_index)
{
  u32 rom_pages = gamepak_rom_pages;
...
#if GAMEPAK_RESIDENT_MODE
  if (gamepak_rom_resident)
    return load_gamepak_page_single(physical_index);
#endif

  if (!gamepak_file_large)
    return gamepak_buffer_count ? &gamepak_buffers[0][0] : NULL;

  gamepak_note_runtime_read(physical_index, true);
...
{
  u8 *primary = load_gamepak_page_single(physical_index);

#if GAMEPAK_PREFETCH_PAGES > 1
  if (!gamepak_rom_resident)
  {
    // SD card I/O dominates on large ROMs. When a 32KB page is faulted in,
    // pull in a small sequential run too. This keeps the LRU granularity at
    // 32KB, but amortizes fseek/fread overhead and helps sequential code/data
    // streams found in many 8MB/16MB games.
    u32 rom_pages = gamepak_rom_pages;
    for (u32 i = 1; i < GAMEPAK_PREFETCH_PAGES; i++)
    {
      u32 next_page = physical_index + i;
      if (next_page >= rom_pages || next_page >= GAMEPAK_MAX_ROM_PAGES)
        break;
      if (gamepak_phy_to_entry[next_page] < 0)
        load_gamepak_page_prefetch(next_page);
    }
  }
#endif
...
void init_gamepak_buffer(void)
{
  unsigned i;
  u32 target_blocks =
    (GAMEPAK_BUFFER_TARGET_SIZE + GAMEPAK_BUFFER_BLOCK_SIZE - 1) /
    GAMEPAK_BUFFER_BLOCK_SIZE;

  if (target_blocks > GAMEPAK_MAX_ROM_PAGES)
    target_blocks = GAMEPAK_MAX_ROM_PAGES;

  // Try to allocate the requested ROM cache budget using smaller backing
  // slabs. 512KB slabs are much less sensitive to PSRAM heap fragmentation
  // than the previous 1MB slabs, while keeping the same total cache budget.
  gamepak_buffer_count = 0;
  gamepak_rom_resident = false;
  gamepak_rom_pages = 0;
  gamepak_raw_size = 0;
  gamepak_initial_pages_loaded = 0;
#if GAMEPAK_PROFILE
  gamepak_runtime_page_faults = 0;
  gamepak_runtime_file_reads = 0;
  gamepak_runtime_prefetch_reads = 0;
  gamepak_runtime_resident_misses = 0;
  gamepak_runtime_read_kb = 0;
#endif
  ...
  for (i = 0; i < GAMEPAK_MAX_ROM_PAGES; i++)
  {
    gamepak_blk_queue[i].next_lru = (u16)(i + 1);
    gamepak_blk_queue[i].phy_rom = -1;
    gamepak_phy_to_entry[i] = -1;
  }

  gamepak_buffer_entry_count = gamepak_buffer_count * GAMEPAK_PAGES_PER_BUFFER_BLOCK;
  if (gamepak_buffer_entry_count > GAMEPAK_MAX_ROM_PAGES)
    gamepak_buffer_entry_count = GAMEPAK_MAX_ROM_PAGES;

  gamepak_lru_head = 0;
  gamepak_lru_tail = gamepak_buffer_entry_count ?
    (u16)(gamepak_buffer_entry_count - 1) : 0;

  printf("GBA ROM cache: %u x %uKB = %uKB, %u pages, prefetch %u, resident-mode %u\n",
    gamepak_buffer_count, gamepak_buffer_blocksize / 1024,
    (gamepak_buffer_count * gamepak_buffer_blocksize) / 1024,
    gamepak_buffer_entry_count, (u32)GAMEPAK_PREFETCH_PAGES,
    (u32)GAMEPAK_RESIDENT_MODE);
}
...
static s32 load_gamepak_raw(const char *name)
{
  ...
  if(gamepak_file_large)
  {
    // Round size to 32KB pages, but keep the exact file size so the final
    // partial backing slab can be padded deterministically.
    fseek(gamepak_file_large, 0, SEEK_END);
    gamepak_raw_size = (u32)ftell(gamepak_file_large);
    fseek(gamepak_file_large, 0, SEEK_SET);
    gamepak_size = (gamepak_raw_size + 0x7FFF) & ~0x7FFF;

    // Load stuff in configurable backing-slab chunks
    u32 buf_blocks = (gamepak_size + gamepak_buffer_blocksize-1) / (gamepak_buffer_blocksize);
    u32 rom_blocks = gamepak_size >> 15;
    gamepak_rom_pages = rom_blocks;
    gamepak_initial_pages_loaded = 0;
    ...
    for (i = 0; i < ldblks; i++)
    {
      // Load one backing slab and map its 32KB pages. Pad the final partial
      // slab so reads from the rounded tail are deterministic.
      u32 file_offset = i * gamepak_buffer_blocksize;
      u32 bytes_left = file_offset < gamepak_raw_size ?
        gamepak_raw_size - file_offset : 0;
      u32 bytes_to_read = bytes_left < gamepak_buffer_blocksize ?
        bytes_left : gamepak_buffer_blocksize;

      memset(gamepak_buffers[i], 0xFF, gamepak_buffer_blocksize);
      if (bytes_to_read)
        fread(gamepak_buffers[i], 1, bytes_to_read, gamepak_file_large);

      for (j = 0; j < GAMEPAK_PAGES_PER_BUFFER_BLOCK &&
                  i*GAMEPAK_PAGES_PER_BUFFER_BLOCK + j < rom_blocks; j++)
      {
        u32 phyn = i*GAMEPAK_PAGES_PER_BUFFER_BLOCK + j;
        u8* blkptr = &gamepak_buffers[i][GAMEPAK_ROM_PAGE_SIZE * j];
        u32 entry = evict_gamepak_page();
        gamepak_blk_queue[entry].phy_rom = (s16)phyn;
        if (phyn < GAMEPAK_MAX_ROM_PAGES)
          gamepak_phy_to_entry[phyn] = (s16)entry;
        gamepak_initial_pages_loaded++;
        // Map it to the read handlers now
...
#if GAMEPAK_RESIDENT_MODE
    gamepak_rom_resident =
      gamepak_buffer_entry_count >= rom_blocks &&
      gamepak_initial_pages_loaded >= rom_blocks;
#else
    gamepak_rom_resident = false;
#endif

    printf("GBA ROM loaded: raw %uKB, rounded %uKB, pages %u/%u, resident %s\n",
      gamepak_raw_size / 1024, gamepak_size / 1024,
      gamepak_initial_pages_loaded, rom_blocks,
      gamepak_rom_resident ? "yes" : "no");

    if (gamepak_rom_resident && gamepak_file_large)
    {
      fclose(gamepak_file_large);
      gamepak_file_large = NULL;
    }
                 ...
                 int force_rtc, int force_rumble, int force_serial)
{
   ...
   // Buffer 0 always has the ROM header and first backing chunk
