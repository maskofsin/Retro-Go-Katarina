...
extern u32 thumb_block_ram_code_region_mask;
void thumb_block_invalidate_address(u32 address);
void thumb_block_invalidate_range(u32 address, u32 bytes);
void thumb_block_flush_cache(void);

static inline u32 thumb_block_region_mask_for_address(u32 address)
{
  if ((address >> 24) == 0x02)
    return THUMB_BLOCK_RAM_CODE_EWRAM;
  if ((address >> 24) == 0x03)
    return THUMB_BLOCK_RAM_CODE_IWRAM;
  return 0;
}

static inline void thumb_block_invalidate_address_if_cached(u32 address)
{
  u32 region_mask = thumb_block_region_mask_for_address(address);

  if (region_mask && (thumb_block_ram_code_region_mask & region_mask))
    thumb_block_invalidate_address(address);
}

static inline u32 thumb_block_region_mask_for_range(u32 address, u32 bytes)
{
  if (!bytes)
    return 0;

  u32 end = address + bytes - 1;

  if (end < address)
    return THUMB_BLOCK_RAM_CODE_EWRAM | THUMB_BLOCK_RAM_CODE_IWRAM;

  u32 start_region = address >> 24;
  u32 end_region = end >> 24;
  u32 mask = 0;

  if (start_region <= 0x02 && end_region >= 0x02)
    mask |= THUMB_BLOCK_RAM_CODE_EWRAM;
  if (start_region <= 0x03 && end_region >= 0x03)
    mask |= THUMB_BLOCK_RAM_CODE_IWRAM;

  return mask;
}

static inline void thumb_block_invalidate_range_if_cached(u32 address, u32 bytes)
{
  u32 region_mask = thumb_block_region_mask_for_range(address, bytes);

  /* Most callers write wholly inside one RAM region, but DMA/direct callers can
   * legally pass a wider range. Check full range intersection so a range that
   * passes through cached EWRAM/IWRAM still invalidates correctly. */
  if ((region_mask & thumb_block_ram_code_region_mask) != 0)
    thumb_block_invalidate_range(address, bytes);
}
